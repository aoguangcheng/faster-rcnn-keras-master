...
class FRCNN(object):
    _defaults = {
       # "model_path": 'C:/faster-rcnn-keras-master/logs/epoch022-loss1.324-rpn0.840-roi0.483.h5',
        "model_path": 'C:/faster-rcnn-keras-master/model_data/epoch048-loss0.935-rpn0.584-roi0.351.h5',
        "classes_path": 'C:/faster-rcnn-keras-master/model_data/voc_classes.txt',
        "confidence": 0.85,
    #    "confidence": 0.9,
    }
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image ,type,zhen):
        img_for_cut = image
...
#   裁剪坐标区域并保存 print(left, top, right, bottom) 11 219 213 340
            dir = "img/cuts/"
            #   type == 1    图片类型
            #  type == 2    视频类型
            cut_path = dir + str(type) + '_' +str(zhen) + '_' + str(i) + '.jpg'
            # cut_path = dir + str(type) + '_' + str(zhen)+ '_' + str(i) + '.jpg'
            print(cut_path)
            img = img_for_cut
            print(img.size)  # （900,599）
            ...
            # 所有满足条件的目标框  thickness 框的粗细
            for i in range(thickness):
                draw.rectangle(
                        [left + i, top + i, right - i, bottom - i],
                        outline=self.colors[int(c)])
                # print坐标位置b'A 1.00'  98 144 500 294   99 145 499 293
