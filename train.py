...
# 设置预训练模型，接着训练之前的最优模型，进一步优化模型，最后将训练完的最优模型保存
    base_net_weights = "model_data/best_weights.h5"
    ...
    for i in range(EPOCH):
        ...
        for iteration, batch in enumerate(rpn_train):
            ...
            if iter_num == EPOCH_LENGTH:
                ...
                if config.verbose:
                    print('The pre_best Epoch is {}. The pre_best loss is {}. The current loss is {}. Saving weights'.format(best_epoch,best_loss,curr_loss))
                if curr_loss < best_loss:
                    best_loss = curr_loss
                    best_epoch = i
                    print('The beat Epoch is {}. The best loss is {}. Saving best_weights'.format(best_epoch,best_loss))
                    model_all.save_weights(best_dir+"/epoch{:03d}-loss{:.3f}-rpn{:.3f}-roi{:.3f}".format(i,curr_loss,loss_rpn_cls+loss_rpn_regr,loss_class_cls+loss_class_regr)+"best.h5")
                    model_all.save_weights(best_save+"/best_weights.h5")
